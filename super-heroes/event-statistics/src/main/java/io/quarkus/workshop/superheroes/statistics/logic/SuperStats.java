package io.quarkus.workshop.superheroes.statistics.logic;

import io.quarkus.workshop.superheroes.statistics.models.Ranking;
import io.quarkus.workshop.superheroes.statistics.models.Score;
import io.quarkus.workshop.superheroes.statistics.models.TeamStats;
...
public class SuperStats {
    ...
    public Multi<Iterable<Score>> computeTopWinners(Multi<Fight> results) {
        return results
            .group()
            .by(fight -> fight.winnerName)
            .onItem()
            .transformToMultiAndMerge(group -> group.onItem().scan(Score::new, this::incrementScore))
            .onItem()
            .transform(topWinners::onNewScore)
